    ...
    export default {
      ...
      data() {
        return {
          ...
          selectType: ALL,
          onlyContent:true,
          desc: {       //往ratingselect组件里传入了该desc对象，会替换ratingselect组件里desc的默认值
            all: '全部',
            positive: '推荐',
            negative: '吐槽'
          }
      ...
      methods: {
        show() {
          this.showFlag = true;
          this.selectType = ALL;    //详情页组件显示时，初始化selectType
          this.onlyContent = false;  //详情页组件显示时，初始化onlyContent
          this.$nextTick(() => {
            if(!this.scroll) {    //使用BScroll时，一定要放在$nextTick方法里
